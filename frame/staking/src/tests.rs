...
fn staking_should_work() {
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		assert_ok!(Session::set_keys(RuntimeOrigin::signed(4), SessionKeys { other: 4.into() }, vec![]));
...
fn nominating_and_rewards_should_work() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_ok!(Staking::bond(RuntimeOrigin::signed(1), 2, 1000, None));
			assert_ok!(Staking::nominate(RuntimeOrigin::signed(2), vec![11, 21, 31]));

			assert_ok!(Staking::bond(RuntimeOrigin::signed(3), 4, 1000, None));
...
fn double_staking_should_fail() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_noop!(
			Staking::bond(RuntimeOrigin::signed(1), 4, arbitrary_value, None),
...
fn double_controlling_should_fail() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_noop!(
			Staking::bond(RuntimeOrigin::signed(3), 2, arbitrary_value, None),
...
fn switching_roles() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		// add 2 nominators
		assert_ok!(Staking::bond(RuntimeOrigin::signed(1), 2, 2000, cmix_id(1u8)));
		assert_ok!(Staking::nominate(RuntimeOrigin::signed(2), vec![11, 5]));

		assert_ok!(Staking::bond(RuntimeOrigin::signed(3), 4, 500, None));
		assert_ok!(Staking::nominate(RuntimeOrigin::signed(4), vec![21, 1]));

		// add a new validator candidate
		assert_ok!(Staking::bond(RuntimeOrigin::signed(5), 6, 1000, cmix_id(5u8)));
		assert_ok!(Staking::validate(RuntimeOrigin::signed(6), ValidatorPrefs::default()));
		assert_ok!(Session::set_keys(RuntimeOrigin::signed(6), SessionKeys { other: 6.into() }, vec![]));
...
fn bond_with_no_staked_value() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			// Can't bond with 1
			assert_noop!(
				Staking::bond(RuntimeOrigin::signed(1), 2, 1, cmix_id(1u8)),
				Error::<Test>::InsufficientBond,
			);
			// bonded with absolute minimum value possible.
			assert_ok!(Staking::bond(RuntimeOrigin::signed(1), 2, 5, cmix_id(1u8)));
...
fn bond_with_little_staked_value_bounded() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			// Stingy validator.
			assert_ok!(Staking::bond(RuntimeOrigin::signed(1), 2, 1, cmix_id(1u8)));
			assert_ok!(Staking::validate(RuntimeOrigin::signed(2), ValidatorPrefs::default()));
			assert_ok!(Session::set_keys(
				RuntimeOrigin::signed(2),
				SessionKeys { other: 2.into() },
...
fn reward_validator_slashing_validator_does_not_overflow() {
	ExtBuilder::default().build_and_execute(|| {
		...
		Staking::bond(RuntimeOrigin::signed(2), 20000, stake - 1, None).unwrap();
	...
	fn only_iterates_max_2_times_max_allowed_len() {
		ExtBuilder::default()
			...
			.add_staker(
				...
				60,
				2_000,
				StakerStatus::<CmixId, AccountId>::Nominator(vec![21, 22, 23, 24, 25]),
			)
			.add_staker(
				71,
				70,
